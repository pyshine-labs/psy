...
def main():
    ...
    parser.add_argument("--backend", choices=("torch", "torch-eager", "vllm"), default="torch-eager",
                        help="torch uses CUDA graphs + flash attention (needs FA built); torch-eager uses native SDPA.")
    ...
    with YuE2Pipeline.from_pretrained(
        ...
        vae_revision=args.vae_revision, device="cuda", backend=args.backend,
    ) as pipe:
        song = pipe(**request)
